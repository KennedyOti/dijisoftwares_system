...
<body class="sb-nav-fixed">
    ...
    <nav class="sb-topnav navbar navbar-expand navbar-dark bg-success">
            ...
            <h3 class="m-0 text-uppercase text-white fw-bold"><i class=" mr-3"></i>Dijisoftwares</h3>
            ...
            <div id="layoutSidenav_nav">
                <nav class="sb-sidenav accordion bg-success" id="sidenavAccordion">
                    <div class="sb-sidenav-menu">
                        <div class="nav mt-5">
                            <a class="nav-link text-white" href="studentDashboard.php">
                                <div class="sb-nav-link-icon"><i class="fas fa-tachometer-alt"></i></div>
                                Dashboard
                            </a>
                            <a class="nav-link text-white" href="findCourse.php">
                            ...
                            <div class="collapse" id="collapseLayouts" aria-labelledby="headingOne" data-bs-parent="#sidenavAccordion text-white">
                                <?php
                                require "source/connection.php";
                                $s_rs = Database::search("
                                    SELECT * 
                                    FROM `student_has_course`
                                    INNER JOIN `course` ON `student_has_course`.course_id = `course`.id
                                    INNER JOIN `course_payment_status` ON `student_has_course`.course_id = `course_payment_status`.course_id
                                    WHERE `student_has_course`.student_id = '" . $sid . "'
                                    AND `course_payment_status`.student_id = '" . $sid . "'
                                    AND `course_payment_status`.status = 1
                                ");
                                $s_num = $s_rs->num_rows;
                                for ($x = 0; $x < $s_num; $x++) {
                                ...
                                ?>
                                    <nav class="sb-sidenav-menu-nested nav">
                                        <a class="nav-link text-white" href="#"><?php echo $s_data["name"]; ?></a>
                                    </nav>
                                <?php
                                }
                                ?>
                            ...
                            <a class="nav-link text-white" href="" onclick="logOut('S');">
                ...
                <tr class="hide-table-padding">
                    <td colspan="3">
                        <div id="collapse<?php echo $stu_data["id"]; ?>" class="collapse in p-3">
                            ...
                            <h6 class="border">Lecture Schedule</h6>
                            <ol>
                                <?php
                ...
                <footer class="py-4 bg-light mt-auto">
                    <div class="container-fluid px-4">
                        <div class="d-flex align-items-center justify-content-between small">
                            <div class="text-muted">Dijisoftwares</div>
                        </div>
                    </div>
                </footer>
            </div>
